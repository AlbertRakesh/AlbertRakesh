void tut8()//the fuction name should be the same as the file name
{
...
// We read the data that we wrote
    ...
    TF1 *fit = new TF1("fit","gaus",0,10);//fit the gaussian fit from 0 to 10;
   ...
   // fit->SetParameter(1);
    ...
    TLegend *leg = new TLegend(0.7,0.7,0.9,0.9);//TLegend(xmin,ymin,xmax,ymax)
    leg->AddEntry(hist, "Measure Data","l");//variable hist , name , l=line
