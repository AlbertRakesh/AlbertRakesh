void tut11()//the fuction name should be the same as the file name
{
...
// We read the data that we wrote
   ...
   // fit->SetParameter(1);
    ...
    TLine *l = new TLine(0,20,15,20);//TLine(xmin,ymin,xmax,ymax) to draw line
    l->SetLineWidth(2);
    l->SetLineColor(kOrange);
    l->Draw();//Draw

    double x0 = 6.3;
    int bin = hist->FindBin(x0);//finding the bin for x0
    double y0 = hist->GetBinContent(bin);//Bin Height
    //Drwaing an arrow
